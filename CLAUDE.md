...
## After done: Phase 6 hardening loop (OVERNIGHT_HANDOFF.md §16)

Once §11 is met, tag the known-good commit `baseline-green` and run the bounded hardening
loop: pick a backlog item → builder on a branch → `pnpm verify` green → reviewer PASS →
merge, else **revert**. **Never regress the baseline. Never weaken or skip a test. Never add
out-of-scope features.** The loop can only improve or hold. Stop when the backlog is empty,
on two no-improvement iterations, or at the budget — always leaving main green + tagged.